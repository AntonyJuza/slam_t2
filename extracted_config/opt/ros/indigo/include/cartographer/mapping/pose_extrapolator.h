...
class PoseExtrapolator {
 ...
 explicit PoseExtrapolator(common::Duration pose_queue_duration,
  ...
  explicit PoseExtrapolator(common::Duration pose_queue_duration,
                            double imu_gravity_time_constant,double max_pitch);
  ...
  {
    //std::cout<<"pitchvel: "<<pitchvel<<std::endl;
    pitchvel_=pitchvel;
    record_time_++;
    if_pitch_ = (fabs(pitchvel_)>max_pitch_? true : false);
    if(if_pitch_){
      pitch_time_++;
      if(pitch_time_ % 200 == 0){
        LOG(INFO)<<"Pitch occur "<<pitch_time_<<" times. occupy of "<<
        ((double)pitch_time_/record_time_)*100<<"%% of total: "<<record_time_;
      }
    }

  }
  bool IfPitchOccur() const {
    return if_pitch_;
  }
