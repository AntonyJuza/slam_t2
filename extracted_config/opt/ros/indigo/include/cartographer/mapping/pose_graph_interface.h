...
class PoseGraphInterface {
 public:
  ...
  struct RebuildInfo {
    int rebuild_times;
    std::chrono::steady_clock::time_point rebuilding_time;
  };
  ...
  struct SharpEdgeInfo {
    ...
    std::vector<std::pair<Eigen::Vector2f,Eigen::Vector2f>> fit_lines;
    std::vector<Eigen::Vector2f> fill_points;
    std::vector<Eigen::Vector2f> eli_points;
};
  ...
  virtual void RebuildMap(const float &hit_prob, const float &miss_prob, const float &dis, const float &theta) = 0;
  ...
  virtual PoseGraphInterface::RebuildInfo GetRebuildInfo() const = 0;
  ...
  virtual void SetLandmarkPose(const std::string& landmark_id,
                               const transform::Rigid3d& global_pose,const bool frozen = false) = 0;

  virtual void SetLandmarkID(const int& real_labelID, const int& marklistIDtofixed)  = 0;
  ...
  virtual SharpEdgeInfo Getsharpedgeinfo() const = 0;
...
virtual std::vector<Constraint> constraints() const = 0;
  ...
  virtual bool GetTimeCorresGlobalPose(const int trajectory_id, const common::Time time, transform::Rigid3d &pose) const = 0;
  virtual void RecordManualLoopPosition(bool record_position) = 0;
  virtual void ManualLoopClosure() = 0;
};
