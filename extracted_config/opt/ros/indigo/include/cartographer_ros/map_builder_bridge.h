...
public:
  ...
  MapBuilderBridge(
    const NodeOptions& node_options, std::unique_ptr<cartographer::mapping::MapBuilderInterface> map_builder, tf2_ros::Buffer* tf_buffer);
  ...
  void SetRebuildMap(const float &hit_prob, const float &miss_prob, const float &dis, const float &theta);
  ...
  void setLandmarkId(const int& real_labelID, const int& marklistIDtofixed);
  bool GetTimeCorresGlobalPose(const int trajectory_id, const cartographer::common::Time time, cartographer::transform::Rigid3d &pose);
  void HandleManualLoopPosition(bool signal)
  {
    return map_builder_->pose_graph()->RecordManualLoopPosition(signal);
  }
  void HandleManualLoopClosure()
  {
    return map_builder_->pose_graph()->ManualLoopClosure();
  }
...
private:
  void OnLocalSlamResult(
    const int trajectory_id, const ::cartographer::common::Time time, const ::cartographer::transform::Rigid3d local_pose, ::cartographer::sensor::RangeData range_data_in_local, const std::unique_ptr<const ::cartographer::mapping::
