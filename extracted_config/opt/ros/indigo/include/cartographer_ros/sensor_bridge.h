...
public:
  explicit SensorBridge(
    int num_subdivisions_per_laser_scan, const std::string& tracking_frame, double lookup_transform_timeout_sec, tf2_ros::Buffer* tf_buffer, ::cartographer::mapping::TrajectoryBuilderInterface* trajectory_builder);
  ...
  void HandleOdometryMessage(const std::string& sensor_id, const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg);
  void HandleNavSatFixMessage(const std::string& sensor_id, const sensor_msgs::NavSatFix::ConstPtr& msg);
  void HandleLandmarkMessage(
    const std::string& sensor_id, const cartographer_ros_msgs::LandmarkList::ConstPtr& msg);
  ...
  void SetPitchVel(const double &pitchvel)
  {
      trajectory_builder_->SetPitchVel(pitchvel);
  }

  void CreatNode(bool flag)
  {
      trajectory_builder_->CreatNode(flag);
  }

private:
  void HandleLaserScan(
    const std::string& sensor_id, ::cartographer::common::Time start_time, const std::string& frame_id, const ::cartographer::sensor::PointCloudWithIntensities& points);
