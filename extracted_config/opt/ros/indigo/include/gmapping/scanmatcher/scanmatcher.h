...
class ScanMatcher{
	public:
		...
		inline unsigned int likelihoodAndScore(double& s, double& l, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings,int& usedbeams,int& maphit,int& drop) const;
...
inline unsigned int ScanMatcher::likelihoodAndScore(double& s, double& l, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings,int& usedbeams,int& maphit,int& drop) const{
	using namespace std;
	l=0;
	s=0;
	maphit=0,drop=0;usedbeams=(m_laserBeams-m_initialBeamsSkip)/(m_likelihoodSkip+1);
	const double * angle=m_laserAngles+m_initialBeamsSkip;
	OrientedPoint lp=p;
	lp.x+=cos(p.theta)*m_laserPose.x-sin(p.theta)*m_laserPose.y;
	lp.y+=sin(p.theta)*m_laserPose.x+cos(p.theta)*m_laserPose.y;
	...
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++){
		...
		if (!found)
		{
		    ...
		    Point psearch;
		    double dr_min;
		    if(cos(lp.theta+*angle)==0.0||sin(lp.theta+*angle)==0.0)dr_min=m_kernelSize;
		    else if(fabs(cos(lp.theta+*angle)/sin(lp.theta+*angle))>1)dr_min=m_kernelSize/cos(lp.theta+*angle);
		    else dr_min=m_kernelSize/sin(lp.theta+*angle);
		    for (int ii=0; fabs(ii*dr_min)<=m_usableRange; ii++)
		    {
		        psearch.x=lp.x+ii*dr_min*cos(lp.theta+*angle);
		        psearch.y=lp.y+ii*dr_min*sin(lp.theta+*angle);
		        //                std::cerr<<"  "<<ii*dr_min<<"  "<<psearch.x<<"  "<<psearch.y<<std::endl;
		        IntPoint ipsearch=map.world2map(psearch);
		        const PointAccumulator& scell=map.cell(ipsearch);
		        if (((double)scell )>m_fullnessThreshold )
		        {
		            maphit++;
		            break;
		        }
...
//	std::cerr<<"**************** "<<usedbeams<<"   c"<<c<<"  maphit"<<maphit<<" drop:"<<drop<<std::endl;
	return c;
}
